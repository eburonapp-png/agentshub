...
export default function App() {
  ...
  const handleToolCall = (agentId: AgentId, name: string, args: any) => {
    if (name === 'generate_mockup') {
      ...
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} executed ${args.language} code in the sandbox environment.`
      });
    } else if (name === 'github_action') {
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} triggered GitHub Action: ${args.action} on ${args.repo}.`
      });
    } else if (name === 'search_web') {
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} searched the web for: "${args.query}".`
      });
    } else if (name === 'send_email') {
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} sent an email to ${args.to} regarding "${args.subject}".`
      });
    } else if (name === 'create_ticket') {
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} created a ${args.priority} priority ticket: "${args.title}".`
      });
    } else if (name === 'analyze_data') {
      addMessage({
        type: 'system',
        content: `${agents[agentId].name} analyzed data from ${args.dataSource} using query: "${args.query}".`
      });
    }
  };
